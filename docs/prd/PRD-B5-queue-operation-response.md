...
当前客户端分别处理 durable ACK、optimistic queue item、`session/queue` 快照和
`awaitingEcho`。ACK 与快照到达顺序不稳定，导致点击 Steering 后 placement 不立即变化，
以及普通 pending 消息被误标为“正在消费”。
...
- [x] **FR5：队列显示语义**：操作响应中的 queued/steering 立即显示；仍在 pending 的 item 不显示“正在消费”；只有 claim 后 item 消失且 USER_MESSAGE 未到达时才显示 `awaitingEcho`。
...
| `packages/renderer/src/features/chat/QueuedMessagesBanner.tsx` | 只根据 pending placement/awaitingEcho 渲染，不再等待 ACK 后二次猜测 |
...
- UI：Steering placement 即时显示，普通 pending/awaitingEcho 文案正确。

...
function pushRecent(folders: string[], path: string): string[] {
    ...
    const exists = folders.some((f) => normalizeFolderPath(f) === norm);
...
export const useWorkspace = create<WorkspaceState>((set, get) => ({
    ...
    restore: async () => {
        ...
        try {
            ...
            if (saved) {
                set({ rootPath: saved, restored: true, recentFolders: pushRecent(recentFolders, saved) });
            } else {
    ...
    removeRecentFolder: (path) => {
        const norm = normalizeFolderPath(path);
        const updated = get().recentFolders.filter((f) => normalizeFolderPath(f) !== norm);

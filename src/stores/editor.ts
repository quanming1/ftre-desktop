...
interface EditorState {
    ...
    openFile: (file: Omit<OpenFile, 'modified' | 'pinned'>) => void;
...
export const useEditor = create<EditorState>((set, get) => ({
    ...
    openFile: (file) => {
        ...
        const newFile: OpenFile = { ...file, modified: false, pinned: false };
        const groups = updateGroup(state.groups, group.id, (g) => ({
            ...g,
    ...
    closeFile: (path) => {
        ...
        set({
            ...
            pendingDiffs: state.pendingDiffs.filter((d) => d.filePath !== path && d.tabPath !== path),
        });
    ...
    refreshFile: (path, newContent) => {
        const state = get();
        const group = getActiveGroup(state);
        const exists = group.openFiles.find((f) => f.path === path);
        if (!exists) return;
    ...
    acceptDiff: (filePath) => {
        ...
        const groups = state.groups.map((g) => {
            ...
            const updatedFiles = hasRealFile
                ? filtered.map((f) => f.path === filePath ? { ...f, content: diff.newContent, modified: false } : f)
                : filtered;
    ...
    handleFileRenamed: (oldPath, newPath, isDir) => {
        ...
        const matchesPath = (filePath: string) =>
            isDir ? filePath.startsWith(oldPath + '/') || filePath.startsWith(oldPath + '\\') : filePath === oldPath;
        ...
        const groups = state.groups.map((g) => {
            ...
            const updatedActive = g.activeFile && matchesPath(g.activeFile)
        ...
        const pendingDiffs = state.pendingDiffs.map((d) =>
            matchesPath(d.filePath) ? { ...d, filePath: updateFilePath(d.filePath), tabPath: `diff:${updateFilePath(d.filePath)}` } : d,
        );
    ...
    handleFileDeleted: (deletedPath, isDir) => {
        ...
        const matchesPath = (filePath: string) =>
            isDir ? filePath === deletedPath || filePath.startsWith(deletedPath + '/') || filePath.startsWith(deletedPath + '\\') : filePath === deletedPath;
        ...
        const pendingDiffs = state.pendingDiffs.filter((d) => !matchesPath(d.filePath) && !matchesPath(d.tabPath));
        ...
        for (const deletedFilePath of deletedFilePaths) {
            const stillOpen = groups.some((g) => g.openFiles.some((f) => f.path === deletedFilePath));
            if (!stillOpen) {
    ...
    createUntitledFile: () => {
        ...
        const newFile: OpenFile = { path, name, language: 'plaintext', content: '', modified: false, pinned: false };
    ...
    closeOtherFiles: (path) => {
        ...
        const removedPaths = new Set(group.openFiles.filter((f) => f.path !== path && !f.pinned).map((f) => f.path));
        ...
        set({
            groups,
            ...syncTopLevel(groups, state.activeGroupId),
            pendingDiffs: state.pendingDiffs.filter((d) => !removedPaths.has(d.filePath) && !removedPaths.has(d.tabPath)),
        });
    ...
    closeFilesToRight: (path) => {
        ...
        const removedPaths = new Set(group.openFiles.filter((f, i) => i > idx && !f.pinned).map((f) => f.path));
    ...
    closeSavedFiles: () => {
        ...
        const removedPaths = new Set(group.openFiles.filter((f) => !f.modified && !f.pinned).map((f) => f.path));
                ...
                const data: PersistedEditorData = {
                    groups: state.groups.map((g) => ({
                        ...
                        openFiles: g.openFiles
                            ...
                            .filter((f) => !f.path.startsWith('diff:') && !f.path.startsWith('untitled:'))
                            .map((f) => ({
                                path: f.path,
                                name: f.name,
                                language: f.language,
    ...
    restore: async (workspace) => {
        ...
        try {
            ...
            // Reload file contents from disk via IPC
            const restoredGroups: EditorGroup[] = [];
            for (const savedGroup of data.groups) {
                const files: OpenFile[] = [];
                for (const meta of savedGroup.openFiles) {
                    try {
                        const result = await window.desktop.fs.readFile(meta.path);
                        if (!result.error) {
                            files.push({
                                path: meta.path,
                                name: meta.name,
                                language: result.language || meta.language,
                                content: result.content,
                                modified: false,
                                pinned: meta.pinned,
                            });
                        }
                        // If file no longer exists, skip it silently
                    } catch {
                        // File inaccessible, skip
                    }
                }

                // Verify activeFile still exists in the restored files
            ...
            // Filter out empty groups, keep at least one

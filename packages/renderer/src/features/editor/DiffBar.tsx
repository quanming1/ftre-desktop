 ...
 */
export function computeDiffStats(original: string, modified: string): { additions: number; deletions: number } {
...
export function DiffBar({ renderSideBySide = true, onToggleMode }: DiffBarProps) {
  const { activeFile, pendingDiffs } = useEditor();
  const diff = pendingDiffs.find((d) => d.tabPath === activeFile);

  if (!diff) return null;

  const { additions, deletions } = computeDiffStats(diff.originalContent, diff.newContent);
  ...
  return (
    ...
    >
      {/* Approximate warning */}
      {diff.isApproximate && (
        <span data-testid="diff-approximate-warning" className="flex items-center gap-1 text-yellow-400" title="此 Diff 基于推断构造，可能不完全准确">
      ...
      <button
        ...
        title={renderSideBySide ? "切换到 inline 模式" : "切换到 side-by-side 模式"}
      >
      ...
      <button
        ...
        onClick={() => handleOpenFile(diff.filePath)}
        className="flex items-center gap-1 text-t-secondary hover:text-t-primary hover:bg-white/5 px-2 py-0.5 rounded transition-colors"
        title="打开源文件"
      >
        <FileText size={14} />

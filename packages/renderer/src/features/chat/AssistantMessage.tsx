...
/**
 * 找到容器内最深的最后一个叶子元素�? * 光标会被 append 到这个元素内部，保证与末尾文本内联显示，
...
export const AssistantMessage = memo(
  function AssistantMessage({ message }: { message: ChatMessage }) {
    ...
    // 按钮点击处理 - 发送选中的按钮文本作为用户消�?    const handleButtonSelect = useCallback((label: string) => {
      import("@/stores/chat").then(({ streamManager }) => {
    ...
    useEffect(() => {
      ...
      // 插入到最深的最后一个叶子元素内�?      const target = findDeepestLastChild(container);

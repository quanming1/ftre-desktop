...
/**
 * �� "Verb target" �ľ�ʽ������ι��ߵ��ã������û�һ�ۿ���"����ʲô"��
 * Verb ��Ӣ�Ķ��ʣ��ο� Codex ��񣩣�target �ǹؼ�������
 *   Ran <command>          / ִ������
 *   Read <file> [Lx-y]     / ���ļ�
 *   Wrote <file> (N lines) / д�ļ�
 *   Edited <file>          / �༭�ļ�
 *   Listed <dir>           / ��Ŀ¼
 *   cd <dir>               / �л�������������������� cd��
 *   Cron <action> "<title>"
...
function buildSummary(
  ...
  switch (n) {
    ...
    case "task": {
      ...
      const head = short ? `Task �� ${short}${prompt.length > 50 ? "��" : ""}` : "Task �� �ɷ�";
...
export const InlineToolCallCard = memo(
  function InlineToolCallCard({
    ...
    // read/edit/write ���ߣ��ɹ�������� metadata ʱ������д� Inspector ���
    ...
    // edit/write������ʹ�� metadata.before/after �� diff Ԥ����read���� metadata.content ���ļ�Ԥ��
    const resultMeta = result?.metadata;
    const hasDiffMeta = !!(resultMeta?.before !== undefined && resultMeta?.after !== undefined && resultMeta?.file);
    const hasReadMeta = !!(resultMeta?.content !== undefined && resultMeta?.file);
    // �ɹ�������п��� metadata �� ����� Inspector��ʧ�� �� ���� inline չ��
    const openInPanelMode = previewableTool && isComplete && !isError && (hasDiffMeta || hasReadMeta);
    ...
    const handleOpenInPanel = useCallback(() => {
      ...
      if ((isEditTool || isWriteTool) && hasDiffMeta && resultMeta) {
        const beforeVal = resultMeta.before ?? "";
        const afterVal = resultMeta.after ?? "";
        const fileVal = resultMeta.file!;
        const addVal = resultMeta.additions ?? 0;
        const delVal = resultMeta.deletions ?? 0;
      ...
      } else if (isReadTool && hasReadMeta && resultMeta) {
    ...
    if (isSetWorkspace) {
      ...
      const unchanged = resultText.match(/������δ�仯:\s*(.+)/);
              ...
              <div className="flex flex-col gap-1">
                {fromPath && (
                  <div className="text-[13px] text-t-ghost">
                    �� <span className="text-t-dim font-mono">{fromPath}</span>
                  </div>
                )}
                <div className="text-[13px] text-t-secondary">
                  {fromPath ? "��" : unchanged ? "δ�仯" : "�л���"}{" "}
      ...
      <div className="py-0.5">
        {isEditTool && hasDiffMeta && resultMeta ? (
          <InlineEditDiff
            ...
            additions={resultMeta.additions ?? 0}
            deletions={resultMeta.deletions ?? 0}
  ...
  return (
    <>
      <button
        ...
        title={`${image.fileName} - ���Ԥ����Ctrl/Cmd+����ⲿ��`}
...
/** �� Skill ��������ȡ frontmatter �� name / description �ʹ����� */
...
/** ���� bash ���ߵ����Э�飺[cwd] / [stderr] / [exit_code] */
...
// ─── Inline edit diff (CodeDiff, collapsible) ──────────────────
    ...
    <div className="py-0.5">
      <div className="flex items-center gap-1.5">
        ...
        >
          ...
          {additions > 0 && (
            <span className="text-[12px] font-mono text-green-600 shrink-0">+{additions}</span>
          )}
          {deletions > 0 && (
        ...
        <button
          ...
          title="Open in panel"
          className="p-1 rounded text-t-ghost hover:text-t-primary hover:bg-hover shrink-0"
        >
...
/**
 * ���� cron ���� list action �� result����˸�ʽ��
 *   - <id> | [����]/[�ѽ���] | <cron expr> | <title>
 *     prompt: <prompt>...
 *     �ϴ�����: <ts> | �ۼ�����: <n> ��
 */
function parseCronList(text: string): CronJobRow[] {
  ...
  for (const raw of lines) {
    ...
    } else if (cur && /�ϴ�����:/.test(line)) {
      const m = line.match(/�ϴ�����:\s*(.+?)\s*\|\s*�ۼ�����:\s*(\d+)/);
      if (m) {
...
function translateCron(expr: string): string {
  ...
  if (day === "*" && month === "*" && week === "*") {
    if (min.startsWith("*/") && hour === "*") return `ÿ${min.slice(2)}����`;
    if (hour.startsWith("*/") && min === "0") return `ÿ${hour.slice(2)}Сʱ`;
    if (hour !== "*" && min !== "*") return `ÿ�� ${hour.padStart(2, "0")}:${min.padStart(2, "0")}`;
    if (hour !== "*") return `ÿ�� ${hour.padStart(2, "0")}:00`;
    if (min !== "*") return `ÿСʱ��${min}��`;
  }

  if (day === "*" && month === "*" && week !== "*") {
    const weekNames = ["��", "һ", "��", "��", "��", "��", "��"];
    const w = weekNames[parseInt(week) % 7] || week;
    if (hour !== "*" && min !== "*") return `ÿ��${w} ${hour.padStart(2, "0")}:${min.padStart(2, "0")}`;
    return `ÿ��${w}`;
  }

  if (day !== "*" && month === "*" && week === "*") {
    if (hour !== "*" && min !== "*") return `ÿ��${day}�� ${hour.padStart(2, "0")}:${min.padStart(2, "0")}`;
    return `ÿ��${day}��`;
  }
        ...
        <div className="overflow-hidden">
          <div className="px-4 pb-3 space-y-2">
            ...
            <div className="flex items-center gap-3 text-[13px] text-t-ghost font-mono pt-1 border-t border-border/20">
              <span>ID: {job.id}</span>
              {job.lastRun && <span>�ϴ�: {job.lastRun}</span>}
              {job.runCount > 0 && <span>���� {job.runCount} ��</span>}
            </div>
          </div>
        </div>
      </div>
...
function CronListDetail({ result, isError }: { result: string; isError: boolean }) {
  if (isError) return <RawPre result={result} isError />;
  if (result.trim() === "��ǰû�ж�ʱ����") {
    return (
      <div className="flex flex-col items-center gap-3 py-4 text-center">
        <Clock size={28} className="text-t-ghost/40" />
        <p className="text-[14px] text-t-ghost italic">��ǰû�ж�ʱ����</p>
      </div>
    );
  }
...
/**
 * task ���ߵķ��ظ�ʽ��[session=<sid>, status=<...>]\n<content>
 */

...
export const InlineToolCallCard = memo(
  function InlineToolCallCard({
    ...
    // read/edit/write 工具：成功完成且有 metadata 时点击整行打开 Inspector 面板
    ...
    // edit/write：优先使用 metadata.before/after 打开 diff 预览；read：用 metadata.content 打开文件预览
    ...
    // 成功完成且有可用 metadata → 点击打开 Inspector；失败 → 回退 inline 展开
    const openInPanelMode = previewableTool && isComplete && !isError && (hasDiffMeta || hasReadMeta);

    // [DIFF-DBG] 诊断日志
    console.log(
      `[DIFF-DBG] InlineToolCallCard metadata check: toolCallId=${block.id}, toolName=${name}` +
        `, isComplete=${isComplete}, isError=${isError}, isEdit=${isEditTool}, isWrite=${isWriteTool}, isRead=${isReadTool}` +
        `, hasResult=${!!result}, hasMetadata=${!!resultMeta}` +
        `, metaKeys=${resultMeta ? Object.keys(resultMeta).join(",") : "none"}` +
        `, beforeLen=${resultMeta?.before?.length ?? -1}, afterLen=${resultMeta?.after?.length ?? -1}` +
        `, beforeEqAfter=${resultMeta?.before === resultMeta?.after}` +
        `, file=${resultMeta?.file}, additions=${resultMeta?.additions}, deletions=${resultMeta?.deletions}` +
        `, hasDiffMeta=${hasDiffMeta}, hasReadMeta=${hasReadMeta}, openInPanelMode=${openInPanelMode}`,
    );

    const handleOpenInPanel = useCallback(() => {
      console.log(
        `[DIFF-DBG] handleOpenInPanel CLICKED: toolCallId=${block.id}` +
          `, openInPanelMode=${openInPanelMode}, isEdit=${isEditTool}, isWrite=${isWriteTool}` +
          `, hasDiffMeta=${hasDiffMeta}, isRead=${isReadTool}, hasReadMeta=${hasReadMeta}`,
      );
      if (!openInPanelMode) {
        console.log("[DIFF-DBG] handleOpenInPanel: bailed — openInPanelMode is false");
        return;
      }
      if ((isEditTool || isWriteTool) && hasDiffMeta && resultMeta) {
        ...
        console.log(
          `[DIFF-DBG] handleOpenInPanel: calling openDiffPreview: toolCallId=${block.id}` +
            `, file=${fileVal}, beforeLen=${beforeVal.length}, afterLen=${afterVal.length}` +
            `, beforeEqAfter=${beforeVal === afterVal}, additions=${addVal}, deletions=${delVal}` +
            `, beforeFirst80=${beforeVal.slice(0, 80)}, afterFirst80=${afterVal.slice(0, 80)}`,
        );

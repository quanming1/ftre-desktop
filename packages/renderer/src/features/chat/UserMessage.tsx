...
/**
 * 渲染单个 code ref chip（和编辑器中�?CodeChipView 样式一致）
 * 点击跳转到对应文件的指定�? */
function CodeChip({
  data,
}: {
...
/**
 * 渲染 parts 数组�?inline 内容
 *
 * parts 类型�? * - text:        渲染�?<span>
 * - code_ref:    渲染�?<CodeChip>（可点击跳转�? * - email:       渲染�?<EmailCard>（邮件卡片）
 * - archive_ref: 渲染�?<ArchiveChip>（归档引用）
 */
...
export const UserMessage = memo(
  function UserMessage({ message }: { message: ChatMessage }) {
    ...
    // 检查是否可以回滚（不在处理中，�?sessionId�?    const canRollback = !isBusy && !!sessionId;

    // 检查是否可�?Fork（消息有 archive_id�?    const archiveId = message.metadata?.archive_id as string | undefined;
    ...
    const handleConfirmRollback = useCallback(
      async (skipCodeRestore: boolean) => {
        ...
        try {
          ...
          // 清理所�?pendingDiffs（关�?diff 标签页，防止 Monaco DiffEditor 报错�?          const editorState = useEditor.getState();
          ...
          // 通过全局事件回填输入框（�?ftre:insert-code-ref 模式一致）
          ...
          useNotification.getState().addNotification({
            level: "info",
            message: `已回�?${result.rolled_back_count} 轮对话`,
          });
        } catch (err) {
          useNotification.getState().addNotification({
            level: "error",
        ...
        <TooltipProvider>
          ...
          >
            ...
            <div className="flex items-center gap-1 pt-2">
              ...
              {/* Fork 按钮 - hover 时显示，仅当�?archive_id �?*/}
              {canFork && (

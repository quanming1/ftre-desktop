...
const EXT_LANG_MAP: Record<string, string> = {
    ...
    sh: 'shell', bash: 'shell', vue: 'vue',
    scss: 'scss', less: 'less', xml: 'xml', sql: 'sql',
    rs: 'rust', go: 'go', java: 'java', c: 'c', cpp: 'cpp',
    h: 'c', hpp: 'cpp', rb: 'ruby', php: 'php', swift: 'swift',
    kt: 'kotlin', toml: 'toml', ini: 'ini', env: 'dotenv',
};
 ...
 */
export async function handleOpenFileAtLine(filePath: string, line: number, col?: number): Promise<void> {
    const fullPath = resolveFilePath(filePath);
    try {
        const result = await window.desktop.fs.readFile(fullPath);
        if (result.error) return;
...
export async function handleShowDiff(message: ToolCallMessage): Promise<void> {
    ...
    if (!filePath || typeof oldString !== 'string' || typeof newString !== 'string') return;
 ...
 */
export async function fetchDiffForInline(message: ToolCallMessage): Promise<InlineDiffResult | null> {
    const callId = message.toolId;
    ...
    const oldString = message.arguments?.oldString as string;
    const newString = message.arguments?.newString as string;
    if (!filePath || typeof oldString !== 'string' || typeof newString !== 'string') return null;

...
import type { AnyMessage, ChatMessage, ToolCallMessage, ActionButtonMessage, CodeRef, MessagePart } from '@/types/chat';
...
interface ChatState {
    ...
    updateToolResult: (toolId: string, result: string, status?: 'completed' | 'error' | 'cancelled') => void;
...
export const useChat = create<ChatState>((set, get) => ({
    ...
    appendAssistantContent: (id, content) => {
        set({
            messages: get().messages.map((m) =>
                m.id === id && 'content' in m ? { ...m, content: m.content + content } : m
            ),
        });
    },
    ...
    clearMessages: () => set({ messages: [], sessionId: null, contextTokens: 0, isStreaming: false, streamingMessageId: null, agentId: 'code_agent' }),
...
export const useMessageById = (id: string) => useChat((s) => getIndex(s.messages).get(id));

...
export class StreamSession {
    ...
    pendingSnapshots = new Map<string, { filePath: string; beforeContent: string }>();
    ...
    addUserMessage(content: string, codeRefs?: CodeRef[], parts?: MessagePart[]): void {
        const msg: ChatMessage = { id: nextId(), role: 'user', content, codeRefs, parts };
    ...
    startAssistantMessage(): string {
        const id = nextId();
        const msg: ChatMessage = { id, role: 'assistant', content: '', streaming: true };
        this.messages = [...this.messages, msg];
    ...
    addToolCall(toolId: string, name: string, args: Record<string, unknown>): string {
        const id = nextId();
    ...
    updateToolResult(toolId: string, result: string, status: 'completed' | 'error' | 'cancelled' = 'completed'): void {
        this.messages = this.messages.map((m) =>
    ...
    addActionButton(label: string, step: string, summary: string): void {
        const msg: ActionButtonMessage = { id: nextId(), role: 'action_button', label, step, summary };
        this.messages = [...this.messages, msg];
        this.emitChange();
    }
    ...
    private updateStreamingToolCallArgs(entry: { argsBuffer: string; messageId: string }): void {
        try {
     ...
     */
    finalizeStreamingToolCall(toolId: string, toolName: string, args: Record<string, unknown>): boolean {
...
class SessionStreamManager {
     ...
     */
    replayInto(sessionId: string, events: Array<{ type: string; data: Record<string, unknown>; metadata?: Record<string, unknown> }>): void {
        const session = this.getOrCreate(sessionId);
        ...
        if (session.isStreaming && session.streamingMessageId) {
            const streamIdx = session.messages.findIndex((m) => m.id === session.streamingMessageId);
            if (streamIdx >= 0) {
        ...
        for (const event of events) {
            ...
            switch (type) {
                case 'user_input': {
                    const parts = data.parts as Array<{ type: string; data: unknown }> | undefined;
                    ...
                    if (parts && Array.isArray(parts)) {
                        ...
                        for (const p of parts) {
                            if (p.type === 'text') {
                                ...
                                const d = p.data as { path: string; name: string; lines: number[]; raw: string };
                                refs.push({
                                    filePath: d.path, fileName: d.name,
                                    startLine: d.lines[0], endLine: d.lines[1],
                                    content: d.raw,
                                });
                            }
                        }
                    ...
                    session.addUserMessage(text, codeRefs, parts as MessagePart[] | undefined);

...
import { useEditor, buildDiffId, buildDiffTabPath, type DiffEntry } from '@/stores/editor';
import { useLayout } from '@/stores/layout';
import { resolveFilePathWithWorkspace, resolveFilePath, normalizePath } from '@/utils/pathUtils';
...
class GlobalEventStream {
    ...
    connect(): void {
        ...
        source.addEventListener('session_started', (e) => {
            try {
                ...
                if (currentWorkspace && normalizePath(workspace) === normalizePath(currentWorkspace)) {
                    useSession.getState().loadSessions(currentWorkspace).then(() => {
        ...
        const eventTypes = [
            'user_input',
            'message', 'message_complete', 'tool_call', 'tool_call_streaming', 'tool_result',
            'tool_cancelled', 'tool_timed_out',
            'usage_update', 'done', 'error', 'interrupt',
            'diff_meta',
        ];
    ...
    private async dispatchEvent(session: StreamSession, eventType: string, payload: Record<string, unknown>): Promise<void> {
        switch (eventType) {
            case 'user_input': {
                // 渲染用户消息
                const content = (payload.content as string) || (payload.text as string) || '';
                const parts = payload.parts as Array<{ type: string; data: unknown }> | undefined;
                let text = content;
                    ...
                    for (const p of parts) {
                        if (p.type === 'text') {
                            ...
                            refs.push({
                                ...
                                startLine: d.lines[0], endLine: d.lines[1], content: d.raw,
                            });
                        }
                    }
                ...
                session.addUserMessage(text, codeRefs, parts as import('@/types/chat').MessagePart[] | undefined);
                break;
            }
            case 'message': {
            ...
            case 'tool_result': {
                ...
                const mappedStatus = toolStatus === 'cancelled' ? 'cancelled'
                    : toolStatus === 'error' ? 'error'
                        : 'completed';
                session.updateToolResult(toolId, (payload.result as string) || '', mappedStatus);
            ...
            case 'error': {
                session.addSystemMessage((payload.message as string) || 'Unknown error');
                session.setStreaming(false);
                break;
            }
            case 'diff_meta': {
                session.attachDiffMetaToLastUserMessage(payload as import('@/types/chat').DiffMeta);
                break;
            }

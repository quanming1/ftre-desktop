...
const api: DesktopAPI = {
  ...
  fs: {
    ...
    showSaveDialog: (opts?: { defaultName?: string }) => ipcRenderer.invoke('fs:showSaveDialog', opts || {}),
    search: (rootPath: string, query: string, options: any) => ipcRenderer.invoke('fs:search', { rootPath, query, options }),
    createFile: (filePath: string) => ipcRenderer.invoke('fs:createFile', { filePath }),
    createFolder: (dirPath: string) => ipcRenderer.invoke('fs:createFolder', { dirPath }),
    rename: (oldPath: string, newPath: string) => ipcRenderer.invoke('fs:rename', { oldPath, newPath }),
    delete: (targetPath: string, isDir: boolean) => ipcRenderer.invoke('fs:delete', { targetPath, isDir }),
    ...
    unwatch: (filePath: string) => ipcRenderer.invoke('fs:unwatch', { filePath }),
    onFileChanged: (callback: (filePath: string) => void) => {
      const handler = (_event: any, payload: { filePath: string }) => callback(payload.filePath);
  ...
  git: {
    ...
    show: (rootPath: string, filePath: string) => ipcRenderer.invoke('git:show', { rootPath, filePath }),
    diffFile: (rootPath: string, filePath: string, status: string, staged: boolean, oldPath?: string) => 
      ipcRenderer.invoke('git:diff-file', { rootPath, filePath, status, staged, oldPath }),
  },
  ...
  terminal: {
    ...
    onData: (callback: (id: number, data: string) => void) => {
      const handler = (_event: any, payload: { id: number; data: string }) => callback(payload.id, payload.data);
      ipcRenderer.on('pty:data', handler);
      return () => ipcRenderer.removeListener('pty:data', handler);
    },
    onExit: (callback: (id: number, exitCode: number) => void) => {
      const handler = (_event: any, payload: { id: number; exitCode: number }) => callback(payload.id, payload.exitCode);

...
import { parseBreadcrumbSegments, type BreadcrumbSegment } from "../utils/breadcrumb-utils";
...
/** Represents a group with open files */
...
export function Breadcrumb({
  ...
  const handleSegmentClick = useCallback(
    async (segment: BreadcrumbSegment, index: number) => {
  ...
  return (
    ...
    >
      {visibleSegments.map((item, i) => {
        ...
        const originalIndex =
          allSegments.length > MAX_VISIBLE_SEGMENTS && i > 1
            ? allSegments.length - (visibleSegments.length - i)
            : i === 0
              ? 0
              : i;

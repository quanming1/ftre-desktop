...
export const MonacoEditor = memo(
  function MonacoEditor({ file, minimapEnabled }: MonacoEditorProps) {
    ...
    useEffect(() => {
      ...
      if (file.loaded && file.content && ed.getValue() === "") {
        ...
        // setValue 会重置光标/滚动位置，需要恢复 viewState
        const savedViewState = editorCore.getViewState(file.path);
        if (savedViewState) {
          ed.restoreViewState(savedViewState);
        }
      }
    }, [file.loaded, file.content, file.path, mounted]);
    ...
    // 统一的 window 事件监听器（合并多个 useEffect，减少开销）
    useEffect(() => {
      const ed = editorRef.current;

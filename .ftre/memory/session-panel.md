...
### 工作区切换
点击 Header 的「切换」→ 展开下拉菜单 → 选择工作区 → `workspaceStore.switchTo(path)` → 刷新会话列表
...
### 工作区切换按钮融入方案
- **位置**: SessionPanel Header 右侧，跨两行（标题+路径）
- **默认隐藏**: hover Header 时才显示，减少视觉干扰
- **下拉菜单**: 复用 TitleBar 的工作区选择逻辑
  - 彩色缩写标识工作区
  - 当前工作区打勾标记
  - 「打开文件夹...」底部入口

...
async function main() {
    ...
    const cachedZip = path.join(cacheDir, `python-${PYTHON_VERSION}-embed-amd64.zip`);
    ...
    log('安装后端 Python 依赖...');
    execSync(`"${pythonExe}" -m pip install -r "${requirementsFile}" --no-warn-script-location --no-cache-dir`, {
        stdio: 'inherit',
        cwd: pythonDir,
    });
    ...
    const backendPackages = ['core', 'storage', 'workspace', 'workflow', 'shared', 'sandbox'];
